...
class Detector:
  ...
  def detect(self, ori_img: np.ndarray, scaled_img: np.ndarray) -> list:
    ...
    # 识别车辆
    # results : Results = self.robot_detector_model.track(scaled_img, persist=True, tracker='botsort.yaml', conf=0.000001)[0]
    results = self.robot_detector_model.predict(scaled_img, conf=0.05)[0]
    ...
    armors_list  = self.armor_model.predict(rois, imgsz=224, conf=0.6)

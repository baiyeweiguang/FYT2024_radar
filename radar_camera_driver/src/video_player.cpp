...
public:
  ...
  : Node("video_player", options), frame_cnt_(0) {
    ...
    std::string camera_info_url =
      this->declare_parameter("camera_info_url", "package://radar_bringup/config/camera_info.yaml");
    ...
    if (!std::filesystem::exists(video_file)) {
      RCLCPP_ERROR(this->get_logger(), "Video file %s does not exist!", video_file.c_str());
      rclcpp::shutdown();
      return;
    }

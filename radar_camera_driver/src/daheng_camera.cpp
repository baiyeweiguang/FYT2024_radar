...
#include "radar_camera_driver/daheng_camera.hpp"
// std
#include <chrono>
#include <cstdlib>
#include <ctime>
...
DahengCameraNode::DahengCameraNode(const rclcpp::NodeOptions &options)
  ...
  camera_info_url_ = this->declare_parameter(
      "camera_info_url", "package://radar_bringup/config/camera_info.yaml");
  frame_id_ =
  ...
  if (pixel_format_ == "mono8") {
    ...
    RCLCPP_ERROR(this->get_logger(), "Unsupported pixel format: %s",
                 pixel_format_.c_str());
  }
  ...
  on_set_parameters_callback_handle_ =
      this->add_on_set_parameters_callback(std::bind(
          &DahengCameraNode::onSetParameters, this, std::placeholders::_1));
  ...
  if (enable_recorder) {
    ...
    std::filesystem::path video_path =
        fs::path(home) / "/fyt2024-log/video/" /
        std::string(std::to_string(std::time(nullptr)) + ".avi");

    recorder_ = std::make_unique<Recorder>(
        video_path, frame_rate_,
  ...
  RCLCPP_INFO(this->get_logger(), "DahengCameraNode has been initialized!");
}
...
DahengCameraNode::~DahengCameraNode() {
  ...
  if (recorder_ != nullptr) {
    ...
    RCLCPP_INFO(this->get_logger(),
                ...
                recorder_->path.c_str());
  }
  RCLCPP_INFO(this->get_logger(), "DahengCameraNode has been destroyed!");
}
...
void DahengCameraNode::timerCallback() {
  ...
  if (dt > 5.0) {
    RCLCPP_WARN(this->get_logger(),
                "Camera is not alive! lost frame for %f seconds", dt);
    close();
  }
}
...
bool DahengCameraNode::open() {
  ...
  if (is_open_) {
    RCLCPP_WARN(this->get_logger(),
                "Daheng Galaxy Camera Device is already opened!");
    close();
  }
  ...
  if ((gx_status_ != GX_STATUS_SUCCESS) || (device_num <= 0)) {
    RCLCPP_WARN(this->get_logger(), "Can't find camera");
    return false;
  }
  RCLCPP_INFO(this->get_logger(), "Found %d devices", device_num);
  // 打开设备
  ...
  if (auto_white_balance_) {
    ...
    GXSetEnum(dev_handle_, GX_ENUM_LIGHT_SOURCE_PRESET,
  ...
  g_callback = std::bind(&DahengCameraNode::onFrameCallbackFun, this,
                         std::placeholders::_1);
  gx_status_ = GXRegisterCaptureCallback(dev_handle_, nullptr, CallbackWrapper);
  if (gx_status_ != GX_STATUS_SUCCESS) {
    RCLCPP_ERROR(this->get_logger(),
                 "Register capture callback function failed!");
    return false;
  }
  ...
  if (gx_status_ != GX_STATUS_SUCCESS) {
    RCLCPP_ERROR(this->get_logger(), "Send start capture command failed!");
    return false;
  }
...
DahengCameraNode::onFrameCallbackFun(GX_FRAME_CALLBACK_PARAM *pFrame) {
  if (pFrame->status == GX_FRAME_STATUS_SUCCESS) {
    // RGB转换
    DxRaw8toRGB24((void *)pFrame->pImgBuf, image_msg_.data.data(),
                  pFrame->nWidth, pFrame->nHeight, RAW2RGB_NEIGHBOUR,
                  static_cast<DX_PIXEL_COLOR_FILTER>(gx_bayer_type_), false);
...
DahengCameraNode::onSetParameters(std::vector<rclcpp::Parameter> parameters) {
  rcl_interfaces::msg::SetParametersResult result;
  result.successful = true;
  for (const auto &param : parameters) {
    if (param.get_name() == "exposure_time") {
      exposure_time_ = param.as_int();
      GXSetFloat(dev_handle_, GX_FLOAT_EXPOSURE_TIME,
                 static_cast<double>(exposure_time_));
      RCLCPP_INFO(this->get_logger(), "Set exposure_time: %d", exposure_time_);
    } else if (param.get_name() == "gain") {
      gain_ = param.as_double();
      GXSetFloat(dev_handle_, GX_FLOAT_GAIN, gain_);
      RCLCPP_INFO(this->get_logger(), "Set gain: %f", gain_);
    }
  }
  return result;
}

...
typedef enum { ADD = 1, UPDATE = 2, DELETE = 3 } GraphOperation;
...
typedef enum { ATTACK_IN = 1, DEFEND_IN = 2, MOVE_TO = 3 } SentryIntention;
...
struct GraphConfig {
  ...
  bool operator==(const GraphConfig &config) {
    return (graphic_id[0] == (uint32_t)config.graphic_id[0] &&
            ...
            end_x == (uint32_t)config.end_x && end_y == (uint32_t)config.end_y);
  }

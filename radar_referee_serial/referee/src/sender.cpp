...
void Sender::sendInteractiveData(int data_cmd_id, int receiver_id,
                                 unsigned char data) {
  ...
  tx_len_ = k_header_length_ + k_cmd_id_length_ +
            static_cast<int>(sizeof(InteractiveData) + k_tail_length_);

  sendSerial(clock_->now(), sizeof(InteractiveData));
}

// void Sender::sendCurrentSentryData(const
// radar_interfaces::CurrentSentryPosData:: data)
// {
//   int data_len;
//   uint8_t tx_data[sizeof(CurrentSentryPosData)] = { 0 };
//   auto current_sentry_pos_data = (CurrentSentryPosData*)tx_data;
//   data_len =
//   static_cast<int>(sizeof(radar_interfaces::CurrentSentryPosData));

//   for (int i = 0; i < 128; i++)
//     tx_buffer_[i] = 0;

//   current_sentry_pos_data->header_data.data_cmd_id =
//   DataCmdId::CURRENT_SENTRY_POSITION_CMD;
//   current_sentry_pos_data->header_data.sender_id = base_.robot_id_;
//   current_sentry_pos_data->header_data.receiver_id = base_.robot_id_ < 100 ?
//   RobotId::RED_SENTRY : RobotId::BLUE_SENTRY;
...
//   pack(tx_buffer_, tx_data,
//   radar_referee::RefereeCmdId::INTERACTIVE_DATA_CMD,
//   sizeof(InteractiveData)); sendSerial(clock_->now(), data_len);
// }
...
void Sender::sendMapSentryData(const radar_referee::MapSentryData &data) {
  ...
  pack(tx_buffer_, tx_data, radar_referee::RefereeCmdId::MAP_SENTRY_CMD,
       sizeof(radar_referee::MapSentryData));
  tx_len_ =
      k_header_length_ + k_cmd_id_length_ +
      static_cast<int>(sizeof(radar_referee::MapSentryData) + k_tail_length_);
...
void Sender::sendCustomInfoData(std::wstring data) {
  ...
  pack(tx_buffer_, tx_data, radar_referee::RefereeCmdId::CUSTOM_INFO_CMD,
       sizeof(radar_referee::CustomInfo));
  tx_len_ = k_header_length_ + k_cmd_id_length_ +
            static_cast<int>(sizeof(radar_referee::ClientMapReceiveData) +
                             k_tail_length_);
...
void Sender::sendRadarCmdData(const radar_referee::RadarCmd &data) {
  int data_len;
  radar_referee::RadarCmd tx_data;
  data_len = static_cast<int>(sizeof(radar_referee::RadarCmd));

  for (int i = 0; i < 128; i++)
    tx_buffer_[i] = 0;
    
  tx_data.header.data_cmd_id = radar_referee::DataCmdId::RADAR_CMD;
  tx_data.header.sender_id = base_.robot_id_;
  tx_data.header.receiver_id = 0x8080;
  tx_data.radar_cmd = data.radar_cmd;

  pack(tx_buffer_, reinterpret_cast<uint8_t *>(&tx_data),
       radar_referee::RefereeCmdId::INTERACTIVE_DATA_CMD, data_len);
  sendSerial(clock_->now(), data_len);
  std::cout<<"[Sender] sendRadarCmdData!";
}

void Sender::sendRadarInteractiveData(
    const radar_referee::ClientMapReceiveData &data) {
  ...
  pack(tx_buffer_, tx_data, radar_referee::RefereeCmdId::CLIENT_MAP_CMD,
       sizeof(radar_referee::ClientMapReceiveData));
  tx_len_ = k_header_length_ + k_cmd_id_length_ +
            static_cast<int>(sizeof(radar_referee::ClientMapReceiveData) +
                             k_tail_length_);
  try {
...
void Sender::pack(uint8_t *tx_buffer, uint8_t *data, int cmd_id,
                  int len) const {
  ...
  memcpy(&tx_buffer[k_header_length_], reinterpret_cast<uint8_t *>(&cmd_id),
         k_cmd_id_length_);
  base_.appendCRC8CheckSum(tx_buffer, k_header_length_);
  memcpy(&tx_buffer[k_header_length_ + k_cmd_id_length_], data, len);
  base_.appendCRC16CheckSum(tx_buffer, k_header_length_ + k_cmd_id_length_ +
                                           len + k_tail_length_);
}
